...
namespace MyHelper.Caches
{
    public class RedisCache : ICache
    {
        ...
        private IDatabase _db => _redisConnection.GetDatabase(_databaseIndex);
        ...
        public object GetCache(string key)
        {
            ...
            if (!redisValue.HasValue)
                return null;
            var valueEntry = JsonConvert.DeserializeObject<ValueInfoEntry>(redisValue.ToString());
            if (valueEntry.TypeName == typeof(string).FullName)
                value = valueEntry.Value;
            else
                value = JsonConvert.DeserializeObject(valueEntry.Value, Type.GetType(valueEntry.TypeName));
        ...
        private void _SetCache(string key, object value, TimeSpan? timeout, ExpireType? expireType)
        {
            ...
            var theValue = JsonConvert.SerializeObject(entry);
            if (timeout == null)
